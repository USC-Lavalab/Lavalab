...
const CarouselSlide = ({ children }: { children: React.ReactNode; icon?: StaticImageData }) => (
  <StyledCarouselSlide>{children}</StyledCarouselSlide>
);

const Carousel = ({ children, titles }: { children: React.ReactNode[]; titles: string[] }) => {
  ...
  return (
    <StyledCarousel>
      ...
      <div className="carouselControls">
        ...
        <Image
          alt=""
          src={currentSlide === children.length - 1 ? RightIconDisabled : RightIconEnabled}
          className={currentSlide === children.length - 1 ? "disabled" : ""}
          onClick={() => {
            if (currentSlide < children.length - 1) setCurrentSlide(currentSlide + 1);
          }}
        />
...
const TiledCarousel = ({ children, titles }: { children: any[]; titles: string[] }) => {
  const [currentSlide, setCurrentSlide] = useState(0);
